...
function RoutePrintSheet({ route, qrDataUrl }: { route: RouteDetail; qrDataUrl: string }) {
  const currentUnit = route.currentUnit || route.unit || "Unidad municipal";
  const oldestDerivation = route.derivations[route.derivations.length - 1];
  const initialReception = [...route.events]
    .reverse()
    .find((event) => /solicitud recibida|recepci[oó]n/i.test(event.title));
  ...
  return <article className="routePrintSheet" aria-hidden="true">
    ...
    <section className="routePrintData">
      <div><small>FECHA</small><strong>{printDate(route.createdAt)} · {printTime(route.createdAt)}</strong></div>
      <div><small>RECEPCIONADO POR</small><strong>{receivedBy}</strong></div>
      <div className="wide"><small>CONSIGNATARIO / DESTINO INICIAL</small><strong>{oldestDerivation?.destinationUnit || currentUnit}</strong></div>
      <div><small>REMITENTE</small><strong>{route.sender}</strong></div>
      <div><small>NÚMERO DE CONTACTO</small><strong>{contact}</strong></div>
      <div className="wide"><small>REFERENCIA</small><strong>{route.description || "Sin referencia adicional"}</strong></div>
      <div className="wide routePrintSubject"><small>ASUNTO</small><strong>{route.title}</strong></div>
    </section>

    <section className="routePrintMovements">
      {Array.from({ length: 5 }, (_, index) => <div className="routePrintRow" key={index}>
        ...
        <div className="routePrintDestination"><h2>DESTINO <b>{String(index + 1).padStart(2, "0")}</b></h2><p><b>Destino:</b></p><p className="short"><b>Fecha:</b><i /><b>Hora:</b></p><p><b>Instrucción:</b> <em>proceder, seguir trámite u otra</em></p><span>FIRMA</span></div>
      </div>)}
    </section>
...
export function RouteWorkflowPanel({
  ...
  const actionTitle = action === "receive" ? "Confirmar recepción" : action === "derive" ? "Derivar a otra unidad" : "Registrar movimiento";
  ...
  return <section className="moduleView routeWorkflow">
    ...
    {selected && <div className="modalBackdrop routeDetailBackdrop no-print" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setSelected(null); }}><section className="routeDetail" role="dialog" aria-modal="true" aria-labelledby="route-detail-title">
      ...
      <div className="routeDetailGrid">
        ...
        <aside className="routeActions"><div className="routeActionHeading"><span>{state === "derivado" ? "PASO 1" : activeForDerivation.has(state) ? "PASO 2" : "EXPEDIENTE"}</span><h3>{actionTitle}</h3><p>{action === "receive" ? "Confirma que tu unidad tiene físicamente la documentación." : action === "derive" ? "Selecciona el siguiente destino e indica la instrucción." : "Registra la actuación realizada sobre el trámite."}</p></div>{allowedActions.length ? <form onSubmit={submitAction}>{allowedActions.length > 1 && <label>Operación<select value={action} onChange={(event) => setAction(event.target.value)}>{allowedActions.map((item) => <option value={item.value} key={item.value}>{item.label}</option>)}</select></label>}{action === "derive" && <label>Unidad de destino<select name="destinationUnitId" required defaultValue=""><option value="" disabled>Selecciona una unidad</option>{selected.units.filter((unit) => unit.id !== selected.currentUnitId).map((unit) => <option value={unit.id} key={unit.id}>{unit.name}</option>)}</select></label>}{action === "deadline" && <label>Nueva fecha límite<input name="dueAt" type="date" required /></label>}<label>{action === "derive" ? "Instrucción" : action === "receive" ? "Observación (opcional)" : "Detalle"}<textarea name="description" rows={4} required={descriptionRequired} minLength={descriptionRequired ? 5 : undefined} placeholder={action === "derive" ? "Ej.: Proceder, emitir informe y seguir trámite" : action === "receive" ? "Estado de la documentación recibida" : "Describe la actuación, instrucción o resultado"} /></label>{!["receive", "derive", "deadline", "archive", "reopen"].includes(action) && <label className="inlineCheck"><input type="checkbox" name="public" defaultChecked={action === "close"} /> Mostrar este movimiento al ciudadano</label>}<button className="primaryAction routeSubmitAction" disabled={submitting}>{submitting ? "Guardando…" : action === "receive" ? "Confirmar recepción" : action === "derive" ? "Derivar trámite →" : "Guardar movimiento"}</button></form> : <p className="emptyState">{hasCurrentUnitAccess ? "No hay acciones disponibles para este estado." : "Este expediente ya está en otra unidad. Puedes consultar su historial, pero solamente la unidad actual puede modificarlo."}</p>}<button className="routeSecondaryPrint" onClick={() => window.print()} disabled={!qrDataUrl}>▤ Imprimir hoja física con QR</button><div className="routeContact"><h4>Datos del remitente</h4><span>{selected.sender}</span>{selected.senderDocument && <span>Documento: {selected.senderDocument}</span>}{selected.senderPhone && <span>Teléfono: {selected.senderPhone}</span>}{selected.senderEmail && <span>{selected.senderEmail}</span>}</div></aside>
      </div>
...
export function RouteCreateModal({ mode, close, succeed, createdCode, units }: { mode: "form" | "success"; close: () => void; succeed: (code: string) => void; createdCode: string; units: MunicipalUnit[] }) {
  ...
  return <div className="modalBackdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) close(); }}>
    {mode === "success" ? <section className="routeModal successModal" role="dialog" aria-modal="true" aria-labelledby="success-title"><div className="successMark">✓</div><h2 id="success-title">Hoja de ruta registrada</h2><p>Se creó el expediente <strong>{createdCode}</strong>. Ya está en la bandeja <b>Recibir</b> de la unidad de destino y desde su detalle puedes imprimir la hoja física con QR.</p><button className="primaryAction" onClick={close}>Ir a la bandeja</button></section>
      : <form className="routeModal" onSubmit={submit} role="dialog" aria-modal="true" aria-labelledby="route-title"><header className="modalHeader"><div><span>SECRETARÍA GENERAL · RECEPCIÓN</span><h2 id="route-title">Nueva hoja de ruta</h2><p>La fecha, hora, código y responsable se registran automáticamente.</p></div><button type="button" onClick={close} aria-label="Cerrar">×</button></header><label>Remitente<input name="remitente" required maxLength={220} placeholder="Nombre de la persona o institución" /></label><div className="formGrid"><label>Documento<input name="documento" maxLength={80} placeholder="CI, NIT o referencia" /></label><label>Número de contacto<input name="telefono" maxLength={40} placeholder="Teléfono o celular" /></label></div><label>Correo electrónico<input name="email" type="email" maxLength={180} placeholder="Opcional" /></label><label>Asunto<input name="asunto" required maxLength={300} placeholder="Resumen principal del trámite" /></label><label>Referencia o detalle<textarea name="descripcion" rows={3} placeholder="Antecedente, documento citado o detalle breve" /></label><div className="formGrid"><label>Tipo<select name="tipo" defaultValue="solicitud_externa"><option value="solicitud_externa">Solicitud externa</option><option value="comunicacion_interna">Comunicación interna</option><option value="solicitud_audiencia">Solicitud de audiencia</option></select></label><label>Prioridad<select name="prioridad" defaultValue="normal"><option value="baja">Baja</option><option value="normal">Normal</option><option value="alta">Alta</option><option value="urgente">Urgente</option></select></label></div><label>Consignatario / destino inicial<select name="unidadCodigo" required defaultValue=""><option value="" disabled>Selecciona una unidad</option>{units.map((unit) => <option value={unit.code} key={unit.id}>{unit.name}</option>)}</select></label><label>Documento inicial (opcional)<input name="file" type="file" accept=".pdf,.doc,.docx,.xls,.xlsx,.jpg,.jpeg,.png" /></label><label className="inlineCheck"><input type="checkbox" name="filePublic" /> Visible para el ciudadano durante el seguimiento</label>{error && <p className="formError" role="alert">{error}</p>}<div className="modalActions"><button type="button" onClick={close}>Cancelar</button><button className="primaryAction" type="submit" disabled={submitting || units.length === 0}>{submitting ? "Registrando…" : "Crear hoja de ruta →"}</button></div></form>}
  </div>;
}
