...
export function RouteWorkflowPanel({
  ...
  return <section className="moduleView routeWorkflow">
    <div className="moduleTitle"><div><span className="routeModuleEyebrow">HOJAS DE RUTA</span><h2>Bandeja de trámites</h2><p>Recibe, procesa y deriva documentos entre unidades.</p></div>{access.hasPermission("sigem.routes.create") ? <button className="primaryAction" onClick={openRouteModal}><UiIcon name="plus" size={17} />Nueva hoja de ruta</button> : null}</div>

    <nav className="routeInboxTabs" aria-label="Bandejas de hojas de ruta">
      <button className={filter === "recibir" ? "selected" : ""} onClick={() => setFilter("recibir")}><span className="routeTabIcon"><UiIcon name="receive" size={20} /></span><span><strong>Recibir</strong><small>Trámites que llegaron a tu unidad</small></span><b>{porRecibir}</b></button>
      <button className={filter === "derivar" ? "selected" : ""} onClick={() => setFilter("derivar")}><span className="routeTabIcon"><UiIcon name="send" size={20} /></span><span><strong>Derivar</strong><small>Recibidos y en gestión</small></span><b>{porDerivar}</b></button>
      <button className={filter === "historial" ? "selected" : ""} onClick={() => setFilter("historial")}><span className="routeTabIcon"><UiIcon name="history" size={20} /></span><span><strong>Historial</strong><small>Todos los expedientes</small></span><b>{allRoutes.length}</b></button>
    </nav>

    <div className="routeSearchBar"><span><UiIcon name="search" size={19} /></span><input aria-label="Buscar por código, asunto o remitente" placeholder="Buscar por código, asunto o remitente…" value={search} onChange={(event) => setSearch(event.target.value)} />{search ? <button onClick={() => setSearch("")}>Limpiar</button> : null}</div>
    ...
    <section className="panel routeInboxPanel" aria-live="polite">{loading ? <p className="loadingState">Actualizando hojas de ruta…</p> : <div className="routeInboxList">{visibleRoutes.length ? visibleRoutes.map((route) => {
      ...
      return <article className="routeInboxRow" key={route.code}>
        <span className={`routeStateGlyph ${isReceive ? "incoming" : isDerive ? "outgoing" : "history"}`}><UiIcon name={isReceive ? "receive" : isDerive ? "send" : "history"} size={20} /></span>
        ...
        <button className="routeRowAction" onClick={() => openDetail(route.id)}>{isReceive ? "Recibir" : isDerive ? "Derivar" : "Ver detalle"}<UiIcon name="chevron" size={15} /></button>
      </article>;
