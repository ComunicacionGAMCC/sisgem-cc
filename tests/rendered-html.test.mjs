...
test("keeps municipal data lazy, protected, and ChatGPT Sites compatible", async () => {
  const [page, agenda, agendaService, agendaApi, agendaActionApi, agendaAttendanceApi, accessServer, medical, dbIndex, schema, orgMigration, listApi, trackingApi, medicalApi, hosting, municipalDate, dateHook, routeService, agendaMigration] = await Promise.all([
    ...
    readFile(new URL("../app/api/agenda/[id]/attendance/route.ts", import.meta.url), "utf8"),
    ...
    readFile(new URL("../db/hojas-ruta.ts", import.meta.url), "utf8"),
    readFile(new URL("../drizzle/0010_cuddly_talos.sql", import.meta.url), "utf8"),
  ]);
  ...
  assert.match(page, /Hola, \{userName\}/);
  assert.doesNotMatch(page, /Buenos días/);
  ...
  assert.match(agenda, /Designar representante/);
  assert.match(agenda, /canDecideCabinetAgendaAttendance/);
  ...
  assert.match(agendaAttendanceApi, /requireCabinetAgendaDecision\(context\)/);
  assert.match(agendaAttendanceApi, /listarDelegadosAgendaActivos/);
  assert.match(agendaService, /accion: delegated \? "designar_representante" : "confirmar_asistencia_alcalde"/);
  assert.match(agendaMigration, /"asistencia" varchar\(30\) DEFAULT 'pendiente'/);
...
test("enforces scoped institutional access with MFA and auditable roles", async () => {
  const [page, accessUi, accessServer, userApi, managedUserApi, accessMigration, bootstrapMigration, managementMigration, mayorMigration, attendanceMigration, directAccessMigration, bootstrapScript] = await Promise.all([
    readFile(new URL("../app/page.tsx", import.meta.url), "utf8"),
    ...
    readFile(
      new URL("../supabase/migrations/20260916223051_agenda_attendance_decisions.sql", import.meta.url),
      "utf8",
    ),
    readFile(
  ...
  assert.match(attendanceMigration, /'sigem\.agenda\.decide'/);
  assert.match(attendanceMigration, /permission\.code = 'sigem\.agenda\.manage'/);
  assert.match(attendanceMigration, /'super_admin', 'sigem_admin', 'sigem_alcalde'/);
